...
const BASE_TOKENS = [
  { symbol: "ETH", name: "Ethereum", price: 2456.78, change: 2.34 },
  { symbol: "USDC", name: "USD Coin", price: 0.9998, change: 0.01 },
  { symbol: "DAI", name: "Dai Stablecoin", price: 1.0001, change: 0.02 },
  { symbol: "BRETT", name: "Brett", price: 0.0065, change: 4.82 },
  { symbol: "TOSHI", name: "Toshi", price: 0.000416, change: 7.35 },
  { symbol: "DEGEN", name: "Degen", price: 0.00093, change: 12.4 },
  { symbol: "AERO", name: "Aerodrome", price: 0.87, change: 3.45 },
];
...
export default function TradeTalk() {
  ...
  const [messages, setMessages] = useState([
    { id: 1, user: "Luna", text: "Connected to Base. Search tokens and trade with real wallet support.", time: "just now" }
  ]);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setPrices(prev => prev.map(t => ({
        ...t,
        price: t.price * (1 + (Math.random() - 0.5) * 0.008),
        change: parseFloat((t.change + (Math.random() - 0.5) * 0.8).toFixed(2))
      })));
    }, 6500);
    ...
    const create = () => {
      ...
      for (let i = 0; i < count; i++) {
        particles.push({
          ...
          size: Math.random() * 2.2 + 0.8, opacity: Math.random() * 0.5 + 0.25
        });
      }
    };
    create();
  ...
  const filteredTokens = prices.filter(t =>
  ...
  const calculateSwap = (amount) => {
    if (!selectedToken || !amount) return setEstimatedOutput("");
    const usdc = prices.find(t => t.symbol === "USDC");
    const output = (parseFloat(amount) * (usdc.price / selectedToken.price)).toFixed(4);
  ...
  const handleShareAlpha = () => {
    const text = "TradeTalk v0.4 on Base — real wallet connection + token discovery. https://tradetalk-ten.vercel.app";
  ...
  return (
    <div style={{ minHeight: "100vh", color: "#fff", fontFamily: "system-ui, -apple-system, sans-serif", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", zIndex: 0, background: "linear-gradient(-45deg, #05070f, #0a0f1e, #0f172a, #1e1135)", backgroundSize: "400% 400%", animation: "gradientShift 32s ease infinite", opacity: 0.85 }} />
      ...
      <div style={{ padding: isMobile ? "14px 16px" : "18px 28px", borderBottom: "1px solid #1a2332", display: "flex", flexDirection: isMobile ? "column" : "row", justifyContent: "space-between", alignItems: isMobile ? "flex-start" : "center", gap: "14px", position: "relative", zIndex: 2 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: "26px", fontWeight: "700" }}>TradeTalk</h1>
        ...
        <div style={{ display: "flex", gap: "10px", flexWrap: "wrap", width: isMobile ? "100%" : "auto" }}>
          ...
          <button 
            ...
            style={{ flex: isMobile ? 1 : "none", background: isConnected ? "#166534" : "#1e2937", color: "#fff", border: "1px solid #334155", padding: "12px 20px", borderRadius: "10px", fontWeight: "600", fontSize: "14.5px" }}
          >
      ...
      <div style={{ padding: isMobile ? "18px 16px" : "28px 28px", maxWidth: "1180px", margin: "0 auto", position: "relative", zIndex: 2 }}>
        ...
        <div style={{ marginBottom: isMobile ? "22px" : "30px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px", flexWrap: "wrap", gap: "12px" }}>
            ...
            <input type="text" placeholder="Search tokens..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ background: "#1e2937", border: "1px solid #334155", borderRadius: "10px", padding: "10px 16px", color: "#fff", width: isMobile ? "100%" : "280px", fontSize: "14.5px" }} />
          </div>
              ...
              <div key={token.symbol} style={{ ...glassStyle, borderRadius: "14px", padding: "16px 18px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div>
                    ...
                    <div style={{ fontSize: "20px", fontWeight: "700", marginTop: "6px" }}>${token.price.toFixed(token.price < 1 ? 6 : 2)}</div>
                  </div>
                  <div style={{ color: token.change >= 0 ? "#22c55e" : "#ef4444", fontWeight: "700", fontSize: "14px" }}>{token.change >= 0 ? "+" : ""}{token.change}%</div>
                </div>
                <button onClick={() => openTrade(token)} style={{ marginTop: "14px", width: "100%", background: "#3b82f6", color: "#fff", border: "none", padding: "10px", borderRadius: "10px", fontWeight: "700", fontSize: "14.5px" }}>
                  Trade {token.symbol}
                </button>
              </div>
            ))}
        ...
        <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: "20px" }}>
          <div style={{ flex: 1, ...glassStyle, borderRadius: "18px", overflow: "hidden", display: "flex", flexDirection: "column", minHeight: isMobile ? "440px" : "500px" }}>
            <div style={{ padding: "16px 20px", borderBottom: "1px solid rgba(148, 163, 184, 0.12)", fontWeight: "600", fontSize: "15.5px" }}>Live Trading Chat — Luna</div>
            <div style={{ flex: 1, padding: "20px", overflowY: "auto", display: "flex", flexDirection: "column", gap: "18px" }}>
              ...
              {isProcessing && <div style={{ color: "#64748b", fontSize: "14px" }}>Processing...</div>}
            </div>
            <div style={{ padding: "15px 18px", borderTop: "1px solid rgba(148, 163, 184, 0.12)", display: "flex", gap: "10px" }}>
              <input value={input} onChange={e => setInput(e.target.value)} onKeyPress={e => e.key === "Enter" && handleSend()} placeholder="Ask Luna..." style={{ flex: 1, background: "rgba(30, 41, 59, 0.6)", border: "1px solid rgba(148, 163, 184, 0.2)", borderRadius: "12px", padding: "14px 18px", color: "#fff", fontSize: "15.5px", outline: "none" }} />
        ...
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: "20px" }}>
          <div style={{ ...glassStyle, borderRadius: "20px", padding: "28px", width: "100%", maxWidth: "420px" }}>
            ...
            <div style={{ marginBottom: "16px" }}>
              ...
              <input type="number" value={tradeAmount} onChange={(e) => { setTradeAmount(e.target.value); calculateSwap(e.target.value); }} placeholder="0.00" style={{ width: "100%", background: "#1e2937", border: "1px solid #334155", borderRadius: "12px", padding: "14px 18px", color: "#fff", fontSize: "20px" }} />
            </div>

            <div style={{ marginBottom: "24px" }}>
              <div style={{ fontSize: "13px", color: "#64748b", marginBottom: "6px" }}>You Receive</div>
            ...
            <button onClick={executeTrade} disabled={!tradeAmount || isProcessing} style={{ width: "100%", background: "#22c55e", color: "#000", border: "none", padding: "16px", borderRadius: "12px", fontWeight: "700", fontSize: "16px", opacity: (!tradeAmount || isProcessing) ? 0.6 : 1 }}>
              {isProcessing ? "Executing..." : "Execute Trade (Gas Sponsored)"}
            </button>
            <div style={{ textAlign: "center", marginTop: "14px", fontSize: "13px", color: "#64748b" }}>Gas sponsored by Base Paymaster</div>
          </div>
        </div>
      )}
    </div>
